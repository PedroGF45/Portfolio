...
// secondary stack is unused now (all core techs promoted to primary)
const secondaryStack: string[] = [];
...
const AboutMe: React.FC = () => {
  ...
  useEffect(() => {
    ...
    for (const img of imgs) {
      const check = () => {
        ...
        if (w < 36 || ratio > 2.2) parent.classList.add('tech-logo--big');
        else parent.classList.remove('tech-logo--big');
      };
  ...
  return (
    <motion.section ref={sectionRef} className="about-section container mx-auto px-4 py-12" initial={{ opacity: 0 }} animate={sectionInView ? { opacity: 1 } : {}} transition={{ duration: 0.6 }}>
      <div className="max-w-4xl mx-auto">
        <h2 className="text-3xl font-bold mb-4">About me</h2>

        {/* Intro: profile photo + short text */}
  <div className="about-intro mb-6 md:flex md:items-stretch md:gap-6">
          <div className="about-avatar w-32 h-32 rounded-full overflow-hidden relative mb-4 md:mb-0">
            <img src="/images/profile-256.webp" alt="Pedro Brito" className="avatar-inner w-full h-full object-cover" />
          </div>
          <div className="md:flex-1">
            <p className="text-base mb-2 about-intro-par">Hi — I'm Pedro. I'm open to software engineering roles broadly, with a preference for data science and AI. I design and implement data-driven products and tools with a focus on machine learning, data engineering, and visualization. I enjoy building space-themed projects and machine learning pipelines that scale.</p>
            <p className="text-sm text-text-300 about-intro-par">I typically work with Python, scientific libraries, and modern web tooling to bring research prototypes into production-ready demos.</p>
          </div>
        </div>
